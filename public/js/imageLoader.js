...
function loadImages() {
  ...
  links.forEach(async (link, index) => {
    ...
    try {
      await new Promise((resolve) => {
        EXIF.getData.call(img, img, function() {
          let imgMetaData = {
          ...
          metaData[index] = imgMetaData; // ← usar índice, no push
          resolve();
        });
      });
    } catch (error) {
...
function openLightbox(startIndex) {
  ...
  description.innerText = metaData[currentIndex].description;
  if (metaData[currentIndex].description) {
    exifDiv.appendChild(description);
  }
